...
public class Query4Test {
	...
	public void testFindAll() {
		...
		List<Article> articles = articleRepository
				.findAll(new Specification<Article>() {
					...
					public Predicate toPredicate(Root<Article> root,
						...
						if (!StringUtils.isEmpty(author)) {
							//拼接作为查询条件
							Predicate predicate = cb.equal(
									root.get("author").as(String.class),
									author);
							list.add(predicate);
						}
...
//		Pageable pageable = PageRequest.of(0, 3);
		...
		//Specification 其实就是对hibernate-jpa的qbc 的封装实现
			...
			public Predicate toPredicate(Root<Article> root,
				...
				if (!StringUtils.isEmpty(title)) {
					...
					Predicate predicate = cb
							.equal(root.get("title").as(String.class), title);
					list.add(predicate);
				}
				if (!StringUtils.isEmpty(author)) {
					//拼接作为查询条件
					Predicate predicate = cb
							.equal(root.get("author").as(String.class), author);
					list.add(predicate);
				}
